  ...
  return (
    <div style={{ maxWidth: 880 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
        ...
        {/* SQL Layer */}
        <Section>
          <SectionLabel>The SQL Layer — Analytical Queries</SectionLabel>
          <p style={{ ...prose, marginBottom: 24 }}>
            Each metric in the dashboard is backed by a named SQL function in{' '}
            <code style={{ background: '#0F172A', padding: '1px 6px', borderRadius: 3, fontSize: 12, color: '#94A3B8' }}>
              server/services/sqlAnalytics.js
            </code>.
            Three queries illustrate the analytical depth: the funnel-gap query, the co-mention matrix, and the sentiment window function.
          </p>
...
ORDER BY mention_rate_pct DESC`}
            ...
            {/* Query 2 */}
            <SqlCard
              title="getCoMentionMatrix() — measures competitive brand association"
              insightLabel="Finding it produces"
              insight="When AI mentions Brex, Ramp co-appears in only 44.8% of those responses. When AI mentions Expensify, Ramp co-appears in 58.1%. This asymmetry reveals that Ramp is weakly associated with Brex in the primary comparison frame — meaning buyers doing a Brex evaluation are less likely to encounter Ramp as an alternative than buyers doing an Expensify evaluation."
              sql={`SELECT
  r.model,
  COUNT(*)                                           AS competitor_mentions,
  SUM(
    CASE WHEN EXISTS (
      SELECT 1 FROM json_each(r.brands_mentioned) je2
      WHERE je2.value = ?   -- primaryBrand
    ) THEN 1 ELSE 0 END
  )                                                  AS co_mentions,
  ROUND(100.0 * SUM(...) / COUNT(*), 1)              AS co_mention_pct
FROM responses r
WHERE r.run_id = ?
  AND EXISTS (
    SELECT 1 FROM json_each(r.brands_mentioned) je1
    WHERE je1.value = ?   -- competitor
  )
GROUP BY r.model`}
            />
            ...
            {/* Query 3 */}
            <SqlCard
              title="getSentimentByBrand() — window function for proportional breakdown"
              insightLabel="Finding it produces"
              insight="Among the 42 responses that mention Ramp, 66.7% carry positive framing, 28.6% neutral, 4.8% negative. The OVER() window function computes each sentiment's share of the total without a subquery — analytically equivalent to what you'd use in Postgres or BigQuery on a large-scale prompt dataset."
              sql={`SELECT
  sentiment,
  COUNT(*)                                              AS count,
  ROUND(
...
function SqlCard({ title, sql, insightLabel, insight }) {
  ...
  return (
    <div style={{ background: '#0F172A', border: '1px solid #1F2937', borderRadius: 10, overflow: 'hidden' }}>
      ...
      >
        ...
        <span style={{ color: '#374151', fontSize: 12, flexShrink: 0, marginLeft: 12 }}>{open ? '▴' : '▾'}</span>
      </button>
      {open && (
        <div style={{ borderTop: '1px solid #1F2937' }}>
          ...
          <pre style={{
            ...
            fontFamily: 'JetBrains Mono, Menlo, monospace', fontSize: 12,
            color: '#7DD3FC', lineHeight: 1.7, background: '#020617',
          }}>
            {sql}
          </pre>
          {/* Insight */}
          <div style={{
            padding: '14px 18px', borderTop: '1px solid #1F2937',
            display: 'flex', gap: 12, alignItems: 'flex-start',
          }}>
            <div style={{
              fontSize: 10, fontWeight: 700, color: '#3B82F6', background: 'rgba(59,130,246,0.1)',
              padding: '2px 7px', borderRadius: 4, flexShrink: 0, marginTop: 1,
              textTransform: 'uppercase', letterSpacing: '0.5px', whiteSpace: 'nowrap',
            }}>
              {insightLabel}
            </div>
            <div style={{ fontSize: 13, color: '#6B7280', lineHeight: 1.65 }}>{insight}</div>
          </div>
        </div>
      )}
    </div>
